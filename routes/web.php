...
Route::get('/planets/{planet}', function ($planet) {
    ...
    // Find the specific planet by name (case-insensitive)
    $foundPlanet = null;
    foreach ($allPlanets as $planetData) {
    ...
    // If planet not found, return 404 or redirect
    if (!$foundPlanet) {
        abort(404, 'Planet not found');
    }
    
    // Pass the single planet to the view
    $planets = [$foundPlanet];
